...
module.exports = function (webpackEnv) {
	...
	const getStyleLoaders = (cssOptions, preProcessor) => {
		const loaders = [
			...
			isEnvProduction && {
				...
				// in production `paths.servedPath` can be a relative path
	...
	return {
		...
		output: {
			...
			filename: isEnvProduction ? 'static/js/[name].js' : isEnvDevelopment && 'static/js/bundle.js',
			// There are also additional JS chunk files if you use code splitting.
			chunkFilename: isEnvProduction ? 'static/js/[name].js' : isEnvDevelopment && 'static/js/[name].js',
			// webpack uses `publicPath` to determine where the app is being served from.
			// It requires a trailing slash, or the file assets will get an incorrect path.
			...
			minimizer: [
				...
				new CssMinimizerPlugin({
					minimizerOptions: {
						...
						// If you want to use a custom CSS processor like postcss
						// you can specify it here. Otherwise, cssnano is used by default.
						processorOptions: {
							parser: 'postcss-safe-parser',
						},
						// Control source map generation here
			...
			rules: [
				...
				},
				{
					...
					oneOf: [
						...
						// Makes sure assets get served by WebpackDevServer.
