...
const diseasesGet = async (req, res, next) => {
  const { code, current_trial_status, name, size, sort, type, ancestor_ids } = req.query;
  ...
  const fileName = ancestor_ids ? `${partialFileName}_${ancestor_fragment}.json` : `${partialFileName}.json`;
