...
const getRequests = async (mockDir) => {
  ...
  const reqLoadPromises = (await readDirAsync(mockDir))
    ...
    .map(async (requestFilename) => {
      const responseFilename = requestFilename.replace(requestFileRegex, 'response.json');

      // No using require cause it will do wonky things sometimes if an
      // error is occurred.
      const reqObj = JSON.parse(await readFileAsync(path.join(mockDir, requestFilename)));

      return { requestObject: reqObj, mockFileName: path.join(mockDir, responseFilename)};
    })

  const requestObjects = (await Promise.all(reqLoadPromises))
    .reduce((ac,curr) => ([
      ...ac,
      curr
    ]),[]);
...
const clinicalTrialsPost = async (req, res, next) => {
  ...
  const matchingMock = requestObjs.find(reqObj => deepEqual(reqObj.requestObject, apiReq));
