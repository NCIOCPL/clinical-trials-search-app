...
const SearchPage = ({ formInit = 'basic' }) => {
	...
	const [{ analyticsName, canonicalHost, siteName, whatAreTrialsUrl, whichTrialsUrl }] = useAppSettings();
	...
	useEffect(() => {
		if (isPageLoadReady) {
			tracking.trackEvent({
				...
				title: `Find NCI-Supported Clinical Trials${pageType === 'advanced' ? ' - Advanced Search' : ''}`,
				metaTitle: `Find NCI-Supported Clinical Trials - ${pageType === 'advanced' ? 'Advanced Search - ' : ''}${siteName}`,
	...
	return (
		<article className="search-page">
			<Helmet>
				<title>{`Find NCI-Supported Clinical Trials - ${pageType === 'advanced' ? 'Advanced Search - ' : ''}${siteName}`}</title>
				<link rel="canonical" href={`https://www.cancer.gov/about-cancer/treatment/clinical-trials/search${pageType === 'basic' ? BasicSearchPagePath() : AdvancedSearchPagePath()}`} />
				<meta name="description" content={`${pageType === 'basic' ? 'F' : 'Use our advanced search to f'}ind an NCI-supported clinical trial—and learn how to locate other research studies—that may be right for you or a loved one.`} />
				<meta property="og:title" content={`Find NCI-Supported Clinical Trials${pageType === 'advanced' ? ' - Advanced Search' : ''}`} />
				<meta property="og:url" content={`https://www.cancer.gov/about-cancer/treatment/clinical-trials/search${pageType === 'basic' ? BasicSearchPagePath() : AdvancedSearchPagePath()}`} />
				<meta property="og:description" content={`${pageType === 'basic' ? 'F' : 'Use our advanced search to f'}ind an NCI-supported clinical trial—and learn how to locate other research studies—that may be right for you or a loved one.`} />
			</Helmet>
			<div ref={sentinelRef} className="search-page__sentinel"></div>
			<h1>Find NCI-Supported Clinical Trials</h1>
			<div className="search-page__header">
				<p>
					NCI-supported clinical trials are those sponsored or otherwise financially supported by NCI. See our guide, <a href={canonicalHost + whichTrialsUrl}>Steps to Find a Clinical Trial</a>, to learn about options for finding trials not included in NCI&apos;s collection.
				</p>
				{renderSearchTip()}
			</div>
