	...
	it('fetches the terms', () => {
		...
		scope
			...
			.query((queryObject) => {
				...
				const codesOk =
					queryObject['code[]'].length === codes.length &&
					queryObject['code[]'].every((code) => codes.includes(code));
				const typesOk =
					queryObject['type[]'].length === types.length &&
					queryObject['type[]'].every((type) => types.includes(type));
				const statusesOk =
					queryObject['current_trial_status[]'].length ===
						ACTIVE_TRIAL_STATUSES.length &&
					queryObject['current_trial_status[]'].every((status) =>
						ACTIVE_TRIAL_STATUSES.includes(status)
					);
