...
describe('getMainType', () => {
	...
	it('throws a 400 error', async () => {
		...
		const scope = nock('http://example.org')
			.get(`/diseases?${querystring.stringify(requestQuery)}`)
			.reply(400);
		await expect(getMainType(client, query.payload.requestParams)).rejects.toThrow('Unexpected status 400 for fetching main types');
		scope.isDone();
	});
	...
	it('throws a 500 error status', async () => {
		...
		const scope = nock('http://example.org')
			.get(`/diseases?${querystring.stringify(requestQuery)}`)
			.reply(500);
		await expect(getMainType(client, query.payload.requestParams)).rejects.toThrow('Unexpected status 500 for fetching main types');
		scope.isDone();
	});
	...
	it('throws a 204 error status', async () => {
		...
		const scope = nock('http://example.org')
			.get(`/diseases?${querystring.stringify(requestQuery)}`)
			.reply(204);
		await expect(getMainType(client, query.payload.requestParams)).rejects.toThrow('Unexpected status 204 for fetching main types');
		scope.isDone();
	});
	...
	it('handles an error thrown by http client', async () => {
		...
		await expect(getMainType(client, query.payload.requestParams)).rejects.toThrow('connection refused');
		scope.isDone();
	});
});
