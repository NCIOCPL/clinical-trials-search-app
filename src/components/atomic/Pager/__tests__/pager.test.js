...
describe('<Pager />', () => {
	...
	it('should display expected pager items and check currently active page on load and on next button click', () => {
		...
		// Suppressing React testing library es-lint rule. Further context within:
		// https://stackoverflow.com/questions/62770973/parent-node-in-react-testing-library

		// eslint-disable-next-line testing-library/no-container,testing-library/no-node-access
		expect(container.querySelectorAll('li.pager__list-item')).toHaveLength(5);
	...
	it('should check for default active page passed, ellipsis display and currently active page on previous button click', () => {
		const { container, rerender } = render(<Pager {...defaultProps} current={5} resultsPerPage={10} totalResults={102} />);
	...
	it('should display correct page neighbours to the left and right of the current page', () => {
		const { container, rerender } = render(<Pager {...defaultProps} currentPageNeighbours={1} resultsPerPage={10} totalResults={102} />);
