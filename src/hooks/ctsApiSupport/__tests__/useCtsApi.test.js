...
import { getClinicalTrialDescriptionAction, getClinicalTrialsAction } from '../../../services/api/actions';
import clinicalTrialsSearchClientFactory, { getClinicalTrialDescription, getClinicalTrials } from '../../../services/api/clinical-trials-search-api';

jest.mock('../../../store/store');
jest.mock('../../../services/api/clinical-trials-search-api/getClinicalTrialDescription.js');
...
const clinicalTrialsSearchClient = clinicalTrialsSearchClientFactory('http://example.org');
...
describe('tests for useCtsApi', () => {
	...
	describe('getClinicalTrials', () => {
		...
		it('should handle an AbortError when calling getClinicalTrials with client and query params', async () => {
			...
			expect(screen.getByText('Aborted: This should not happen')).toBeInTheDocument();
			expect(getClinicalTrials.mock.calls).toHaveLength(1);
			expect(getClinicalTrials.mock.calls[0][1]).toEqual(expected);
		});
	...
	describe('getClinicalTrialDescription', () => {
		it('should fetch the data with with trialId for getClinicalTrialDescription', async () => {
			...
			expect(screen.getByText('Payload[0]-trialId: NCI-2018-02261')).toBeInTheDocument();
