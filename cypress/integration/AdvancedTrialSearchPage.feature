...
Scenario: Click event fires when user starts typing in Primary Cancer Type/Condition field
  ...
  And user types "c" in "ct-searchTerm" field
  Then there should be an analytics event with the following details
      | key                | value                                              |
      | type               | Other                                              |
...
Scenario: Click event fires when user clicks on Find Trials button from Advanced Search form
  ...
  Then the page title is "Find NCI-Supported Clinical Trials"
  And browser waits
  When user scrolls down to "Trial Type"
...
Scenario: Page Load event fires when user fills out Cancer Type Condition fields on Advanced Form
  ...
  Then there should be an analytics event with the following details
    ...
    | page.additionalDetails.formData.stages     | C94774                                                             |
    | page.additionalDetails.formData.location   | search-location-all                                                |
    | page.additionalDetails.formData.cancerType | C4872                                                              |
...
Scenario: Page Load event fires when user fills out Trial Type field and searches
  ...
  Then there should be an analytics event with the following details
      ...
      | page.additionalDetails.analyticsName              | Clinical Trials                                                                                        |
      | page.additionalDetails.formType                   | advanced                                                                                               |
      | page.additionalDetails.numResults                 | (int)501                                                                                               |
      | page.additionalDetails.status                     | success                                                                                                |
      | page.additionalDetails.formData.healthyVolunteers | true                                                                                                   |
      | page.additionalDetails.formData.location          | search-location-all                                                                                    |
      | page.additionalDetails.formData.trialTypes        | treatment,prevention,supportive_care,health_services_research,diagnostic,screening,basic_science,other |
 ...
 Scenario: Page Load event fires when user fills out Drug Treatment and Trial Phase fields and searches
  ...
  Then there should be an analytics event with the following details
      ...
      | page.additionalDetails.numResults           | (int)2                                                             |
      | page.additionalDetails.status               | success                                                            |
      | page.additionalDetails.formData.drugs       | C599                                                               |
      | page.additionalDetails.formData.location    | search-location-all                                                |
      | page.additionalDetails.formData.trialPhases | i,ii,iii,iv                                                        |
      | page.additionalDetails.formData.treatments  | C18309                                                             |
...
Scenario: Page Load event fires when user fills out Trial Id, Lead organization and investigator fields and searches
 ...
 Scenario: Click event fires when user clicks on Modify Search criteria button
  ...
  Then there should be an analytics event with the following details
      ...
      | data.analyticsName | Clinical Trials                                             |
      | data.formType      | advanced                                                    |
      | data.source        | modify_search_criteria_link                                 | **/// this is not required in the old base code, so keeping or removing?**
...
Scenario: Click event fires when user clicks on switch to Basic search form (search tip)
  Given "ctsTitle" is set to "Find NCI-Supported Clinical Trials"
  And "baseHost" is set to "http://localhost:3000"
  And "canonicalHost" is set to "https://www.cancer.gov"
  And "siteName" is set to "National Cancer Institute"
  ...
  Then the page title is "Find NCI-Supported Clinical Trials"
  And browser waits
  And user clicks on "basic search" link
  Then there should be an analytics event with the following details
      | key                | value                                          |
      | type               | Other                                          |
      | event              | ClinicalTrialsSearchApp:Other:TryNewSearch     |
      | linkName           | CTSTryNewSearchClick                           |
      | data.analyticsName | Clinical Trials                                |
      | data.formType      | basic                                          |

Scenario: Click event fires when user clicks on switch to Advanced search form (search tip)
  Given "ctsTitle" is set to "Find NCI-Supported Clinical Trials"
  And "baseHost" is set to "http://localhost:3000"
  And "canonicalHost" is set to "https://www.cancer.gov"
  And "siteName" is set to "National Cancer Institute"
